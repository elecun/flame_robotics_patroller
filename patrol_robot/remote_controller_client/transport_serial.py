...
import time, logging
from .remote_controller_client import ByteStr, RCError

logger = logging.getLogger("rc.serial")
...
fh = logging.FileHandler("rc_serial_log.csv", mode="a", encoding="utf-8")
...
class SerialTransport:
    ...
    def write(self, data: ByteStr) -> int:
        if not self._ser:
            raise RCError("Serial not open")
        try:
            written = self._ser.write(data)
            self._log_io("write", data)
            return written
        except Exception as e:
            raise RCError(f"Serial write failed: {e}")
    ...
    def read(self, n: int) -> ByteStr:
        ...
        try:
            ...
            raise RCError(f"Serial read failed: {e}")
    ...
    def in_waiting(self) -> int:
        ...
        try:
            ...
            raise RCError(f"Serial in_waiting failed: {e}")
